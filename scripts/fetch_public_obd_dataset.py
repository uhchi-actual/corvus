...
WARM_COOLANT_MIN_C = 82.0
PUBLIC_SEED_SOURCES = {
    "public_obd_kit_normal.csv": {
        "source_file": "2018-03-26_Seat_Leon_S_RT_Stau.csv",
...
def main() -> None:
    ...
    if args.all_dashboard_seeds:
        for filename, source in PUBLIC_SEED_SOURCES.items():
            csv_path = _extract_source_csv(archive_path, work_dir, source["source_file"])
            seed_path = output_path.parent / filename
            _write_normalized_seed(
                csv_path,
                seed_path,
                args.max_rows,
                source.get("min_warm_coolant_c"),
            )
        return
...
def _write_normalized_seed(
    csv_path: Path,
    output_path: Path,
    max_rows: int,
    min_warm_coolant_c: float | None = None,
) -> None:
    rows = _normalized_rows(csv_path, max_rows, min_warm_coolant_c)
    if not rows:
...
def _normalized_rows(
    ...
    warm_started = min_warm_coolant_c is None
    ...
    with csv_path.open(newline="", encoding="utf-8-sig") as handle:
        ...
        for row in reader:
            ...
            if not warm_started:
                coolant = float(last_values["coolant_temp_c"])
                if coolant < min_warm_coolant_c:
                    continue
                warm_started = True
